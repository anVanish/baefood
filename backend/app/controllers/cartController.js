...
exports.getCart = async (req, res, next) => {
    try{
        ...
        const carts = await Carts.find({userId})
            .populate('foodId', 'name description imageLink chef')
        res.json(new ApiResponse().setData('carts', carts).setData('total', carts.length))
    } catch(error){
        next(error)
    }
}
...
exports.addToCart = async (req, res, next) => {
    try{
        ...
        res.json(new ApiResponse().setData('cartCount', cartCount).setSuccess('Food added to cart'))
    }catch(error){
        next(error)
    }
}
...
exports.deleteFromCart = async (req, res, next) => {
    try{
        ...
        res.json(new ApiResponse().setSuccess('Cart item deleted'))
    }catch(error){
        next(error)
    }
}