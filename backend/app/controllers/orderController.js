...
exports.getServeTime = async (req, res, next) => {
    try{
        ...
        const nextDate = new Date(currentYear, currentMonth, currentDay + day, 0, 0, 0, 0)
        ...
        if (day === 0) {
            if (currentHour <= times.breakfast) serveDates.breakfast = breakfast
...
exports.addMyOrder = async (req, res, next) => {
    try{
        ...
        res.json(new ApiResponse().setSuccess('Ordered successfully').setData('order', order))
    }catch(error){
        next(error)
    }
}
